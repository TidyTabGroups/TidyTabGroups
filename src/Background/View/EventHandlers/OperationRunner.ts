...
export async function runActiveWindowOperation(
  ...
  const { isValid, activeWindow, windowUpToDate } = await validateWindowUpToDateAndActiveWindow(
    windowId
  );
  if (!isValid) {
    return;
...
export async function runActiveWindowTabGroupOperation<T extends Partial<ChromeTabGroupWithId>>(
  ...
  const { isValid, activeWindow, tabGroupUpToDate } = await validateTabGroupUpToDateAndActiveWindow(
    tabGroupId,
    requiredPropertiesToMatch
  );
  if (!isValid) {
    return;
  }
...
export async function runActiveWindowTabOperation(
  ...
  const { isValid, activeWindow, tabUpToDate } = await validateTabUpToDateAndActiveWindow(
    tabId,
    requiredPropertiesToMatch
  );
  if (!isValid) {
